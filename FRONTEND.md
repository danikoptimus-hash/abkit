...
`SourceTag` в `DatasetSelect.tsx`), Refresh для `source=sql` датасетов
(перевыполняет сохраненный SQL, за подтверждающим Modal) в ДВУХ местах —
hover-reveal иконка в колонке Actions (`className="hover-actions"`, как
остальные экшены строк, см. `ExperimentsList.tsx`) и лейбл-кнопка «Refresh
from source» в drawer превью рядом с сохраненным SQL-запросом; оба видны
только Editor+ (`hasMinRole`). Drawer также показывает сохраненный
SQL-запрос + «Last fetched».
