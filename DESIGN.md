...
class TestResult:
    ...
    variance_reduction: float | None # достигнутое снижение дисперсии (CUPED/RemoveOutliers/пост-страт)
    warnings: list[str]
...
3. **Расчет мощности (design/power.py).** Оценка дисперсии каждой метрики по историческим данным (pre-period колонки; если pre-period нет — CLI просит либо колонку-прокси, либо дисперсию руками). Два режима: задан `mde` → требуемый sample size (сверка с доступным, при нехватке — сообщение с вариантами: достижимый MDE, требуемая длительность); задан `sample_size`/«все доступные» → достижимый MDE по каждой метрике. Если у метрики есть `pre_col`: посчитать ρ(metric, pre) и выдать MDE в двух вариантах — без CUPED и с CUPED (дисперсия × (1−ρ²)) — одинаково для continuous И binary (баг, где binary-метрики молча пропускали CUPED-ветку, исправлен: до фикса `mde_rel_cuped`/`sample_size_per_group_cuped` оставались `None`, хотя ρ уже считался). Формулы без CUPED: для continuous — стандартная двухвыборочная (z-приближение), для binary — точный тест по пропорциям (`sample_size_binary`/`mde_binary`), для ratio — дисперсия дельта-методом. С CUPED: для continuous — та же z-формула на дисперсии × (1−ρ²); для binary — CUPED-остаток уже не строго 0/1, поэтому вместо точного теста по пропорциям используют нормальное приближение на дисперсии Бернулли p(1−p) × (1−ρ²) (`abkit/design/power.py::mde_binary_cuped`/`sample_size_binary_cuped`, дисперсия — `binary_variance`). Учет числа групп (сравнений с контролем) и будущей поправки на множественность по primary-метрикам. **Secondary-метрики (`MetricConfig.role == "secondary"`) всегда получают собственный достижимый MDE** (`_fill_achievable_mde`, та же формула, что и для ветки `sample_size`/«все доступные», но на актуальном финальном n на группу — `n_control_available`, поскольку `run_split()` всегда сплитует весь пул кандидатов целиком, `config.sample_size` его никогда фактически не подвыбирает) — независимо от режима `mde`/`sample_size` primary-метрик; баг, где secondary дословно копировала `config.mde` как свою цель (игнорируя собственные baseline/дисперсию), исправлен (`compute_power_results`, `abkit/experiment.py`). Поведение primary-метрик не меняется. Возвращаемый словарь `compute_power_results()` отсортирован (стабильно) так, что primary-метрики идут первыми, secondary — после (порядок объявления сохраняется внутри роли) — единая точка правки для порядка строк и в персистентном `computed.power` (JSONB конфига), и в `design_report.html`'s power_rows. Отображение (Design tab, design_report.html): "n per group" переименован в "Required n per group" (с тултипом) и округляется вверх (`ceil`, не `round`/`floor` — реальное требование никогда не бывает меньше показанного числа); под таблицей — строка фактических размеров групп по имени и предупреждение, если у метрики required n превышает наименьшую фактическую группу. Метрика с относительным MDE > 100% базового значения помечается некритичным (informational) предупреждением — конфигурация может требовать нереалистично большого эффекта для этой метрики.

   **Strata power check** (`abkit/experiment.py::compute_strata_power_rows`, вызывается только препросмотром визарда `abkit/jobs.py::preview_strata_power`, в `config.yaml` не сохраняется) — не то же самое, что расчет мощности выше: там MDE считается по всей выборке, здесь — ВНУТРИ каждой страты по отдельности при уже выбранных пользователем пропорциях групп, отдельно по каждому измерению стратификации и по их декартовой комбинации, только для primary-метрик типа continuous/binary. Статус (`ok`/`weak`/`insufficient`) — эвристика: `weak`, если MDE внутри страты > 2× общего MDE (`WEAK_STRATUM_MDE_MULTIPLIER`), `insufficient` при n < `MIN_STRATUM_N_FOR_POWER_CHECK` (20).
...
3. **Пайплайн по метрикам.** Для каждой метрики цепочка шагов: из аргумента `methods`, иначе из `MetricConfig.default_methods`, иначе дефолт по типу: continuous → [WelchTTest] (+CUPED если есть pre_col), binary → [ZTestProportions], ratio → [DeltaMethodTTest]. Если сплит был stratified или в данных есть страты — PostStratification доступна как шаг. При >2 групп: каждая тритмент-группа против контроля, это учитывается в множественности. React-UI выставляет `methods` наружу как явный выбор метода анализа на Analysis-таб: per-metric МУЛЬТИселект (только применимые к типу метрики методы — без pre_col CUPED не предлагается, continuous не видит Z-test пропорций), первый выбранный id — designed/primary, остальные — comparison-набор ЦЕЛИКОМ заданный пользователем (замена старой связки `compare_methods: bool` + фиксированный `compare_methods_chains()` на Analysis-табе — то есть 2+ выбранных метода = сравнение, 1 = чистый расчет). Backend переводит id методов (`welch`/`cuped_welch`/`mann_whitney`/... — `abkit/experiment.py::steps_for_method_id`) в цепочки Step перед вызовом `analyze()`: первый id → `methods` (designed), остальные → новый параметр `extra_methods: dict[str, list[list[Step]]]` (per-metric явный список extra-цепочек, ПОДМЕНЯЕТ `compare_methods_chains()`/флаг `compare_methods` для метрик, присутствующих в словаре — остальные вызовы `analyze()` без `extra_methods`, например CLI и симуляции валидации, продолжают использовать старый bool-флаг без изменений). "Designed vs manually selected" не хранится отдельным полем — определяется сравнением `TestResult.method` со строкой, которую дал бы `recommended_method_id()` для типа/pre_col метрики.
4. **compare_methods=True.** Для каждой continuous-метрики дополнительно посчитать стандартный набор альтернатив: Welch сырой, Welch+trim 1%, Welch+CUPED (если есть pre_col), Bootstrap BCa, Mann-Whitney. Для binary (item 3) — свой набор: Chi-square test (независимая реализационная кросс-проверка Z-теста, для таблицы 2x2 chi2 = z² без поправки Йетса), Bootstrap percentile (не BCa — bias-коррекция BCa шумит у пропорций около 0/1), CUPED+Welch t-test на 0/1 (если есть pre_col, та же механика, что для continuous). Mann-Whitney и outlier-методы для binary осознанно не включены — вырождены/неприменимы для 0/1-серии. Ratio-метрики compare_methods пока не покрывает (тот же открытый вопрос для continuous и binary). Дизайн-цепочка помечается `is_designed_method=True` — решение принимается только по ней, остальные для устойчивости выводов.
5. **Множественность (analysis/multiple_testing.py).** Поправка (`holm` по умолчанию) применяется к семейству: primary-метрики × сравнения с контролем, только по designed-цепочкам. Secondary и любые метрики, отсутствовавшие в дизайне, — exploratory: показываются с сырыми и скорректированными p-value и пометкой, в вердикт не входят.
   ...
   **Порядок строк (`AnalysisResults.__init__`)** — стабильная сортировка `self._results` по роли метрики: primary раньше secondary, порядок объявления в конфиге сохраняется внутри роли (и, соответственно, designed-цепочка раньше своих compare_methods-альтернатив — это уже было так по порядку добавления в `Experiment.analyze()`, просто раньше терялось при пересортировке ниже). Единая точка правки — из нее выводятся `.metrics`/`_by_metric` (порядок карточек-вердиктов и секций `report.html`) и `to_json()`/API-ответ (порядок в `results.results`, который читает фронт). `detailed_rows()` (питает и Detailed Results Table в `report.html`, и CSV-эквивалент на фронте) раньше пересортировывал строки алфавитно по (metric, method) — это отменено (порядок строк = порядок `self._results`), поскольку такая пересортировка ломала как раз этот primary-first порядок и вдобавок могла поставить alt-метод перед его же designed-строкой, если имя метода алфавитно меньше. Фронт (`DetailedResultsTable.tsx`, `AnalyzeResults.tsx::VerdictCards`) держит собственную (стабильную, только по роли) пересортировку — защита для уже проанализированных экспериментов, чей `results.json` был записан до этого фикса и не пересчитывается задним числом.

   **Сегменты по отдельным измерениям стратификации** (`Experiment.analyze()`, `segment_results_by_dimension`): при 2+ страта-колонках комбинированная `stratum` (уже склеенная через `|` на этапе дизайна) дешево раскладывается обратно на отдельные значения по каждой колонке (`self.config.strata` как индекс), без повторного чтения/бакетирования сырых данных — строки, слипшиеся в `_other_`/`_all_`, из пер-измеренческой раскладки исключаются (в комбинированной остаются). Результат — `{dimension_label: {treat_name: [(seg_value, TestResult), ...]}}`, дополнительно к уже существовавшей комбинированной раскладке (ключ `"dim1 × dim2"`), а не вместо неё. Используется UI-селектором «Segment by» (Results-таб) и параллельной секцией в `report.html`; на пометку exploratory и отсутствие поправки на множественность (см. п.5) это не влияет — распространяется на все измерения одинаково.
...
- **CUPED**: θ = cov(Y, X_pre)/var(X_pre) на объединенных данных; Y' = Y − θ(X_pre − mean(X_pre)). NaN в ковариате → импутация средним + warning с долей пропусков. В TestResult — фактическое снижение дисперсии (`ctx.variance_reduction`, `1 - var(Y')/var(Y)` на объединенных данных, ≈ ρ²).
- **PostStratification**: оценка эффекта как взвешенная по стратам разность средних, дисперсия — по формуле стратифицированной оценки; `ctx.variance_reduction` — та же стратифицированная дисперсия против наивной (нестратифицированной) дисперсии разности средних на тех же данных.
- **RemoveOutliers(q)/Winsorize(q)**: порог по квантилю объединенных данных, применяется к обеим группам; в TestResult — n_removed по группам. RemoveOutliers дополнительно считает `ctx.variance_reduction` (дисперсия объединенных значений до/после обрезки) — Winsorize не обрезает наблюдения, а клипует, поэтому variance_reduction для него не считается.
- Детальная таблица результатов (`AnalysisResults.detailed_rows()`) размечает технику снижения дисперсии по строке метода (`method_display_name()`'s префикс-цепочка): "CUPED (14.2%)", "PostStrat (20.0%)", "Outlier removal (37.0%)"; методы без механики снижения дисперсии (голый Welch/Z-test/Mann-Whitney/bootstrap) — "—".
...
(report.html больше не содержит аппендикса с config.yaml/версией/seed — эти
детали и так доступны в UI; убран как избыточный дубль. design_report.html
свой отдельный, более короткий аппендикс сохраняет — см. ниже.)
