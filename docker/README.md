...
**From SQL**: выберите подключение, введите `SELECT`-запрос, нажмите **Preview**
(первые 100 строк без сохранения), затем **Create dataset** — запрос
выполняется в фоне (индикатор прогресса), результат стримится чанками и
сохраняется как parquet-файл, обычный датасет для дизайна/анализа/валидации.
Лимиты — `ABKIT_SQL_MAX_ROWS` (по умолчанию 5 000 000 строк, датасет
обрезается с пометкой "truncated") и `ABKIT_SQL_TIMEOUT_SEC` (по умолчанию 300
секунд). SQL-датасеты можно обновить (Editor+) кнопкой **Refresh** — иконка в
колонке Actions списка датасетов (появляется при наведении на строку) либо
кнопка "Refresh from source" в превью-drawer датасета, рядом с сохраненным
SQL-запросом. Оба места сначала показывают подтверждающий диалог ("This will
replace the stored snapshot with fresh data from the source"), затем
выполняют запрос заново в фоне (джоб с прогрессом) — `n_rows`, колонки и
`fetched_at` обновляются только при успехе; при ошибке источника (подключение
недоступно, таблица удалена) старый снапшот остается нетронутым, ошибка
