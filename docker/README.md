...
смена таблицы больше не перезаписывает ваш текст. Дальше — **Preview** (первые
...
- **Edit** (владелец датасета или Admin) — та же форма (общие компоненты с
  ...
  При открытии Schema/Table пытаются определить себя разбором сохраненного
  SQL (простой случай `FROM schema.table`; для JOIN/подзапросов остаются
  пустыми — это нормально). Смена Connection сбрасывает Schema/Table (другой
  источник — другой список); выбор таблицы предлагает заменить SQL на
  `SELECT * FROM ...` через диалог подтверждения (в отличие от формы
  создания, где это происходит молча, только пока запрос не редактировался
  вручную, — здесь редактор с самого открытия содержит уже сохраненный,
  то есть "ручной", запрос). Внизу модала — секция **Data preview**
