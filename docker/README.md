...
- **Refresh** (только `source=sql`, Editor+) — перевыполняет сохраненный SQL,
  ...
  по-человечески.
- **Edit** (владелец датасета или Admin) — та же форма, что при создании,
  с предзаполненными полями: для `source=upload`/`demo` редактируется только
  имя (данные файла не меняются); для `source=sql` — имя, подключение,
  запрос. Если запрос или подключение изменились, сохранение переспрашивает
  подтверждение ("Saving will re-fetch data with the new query...", с числом
  затронутых экспериментов) и перевыполняет запрос тем же механизмом, что
  Refresh.
