...
def get_help_text(chart_type: str) -> str:
    """Markdown-текст «Что показано / Как читать / Когда что-то не так» для
...
def get_warning(chart_type: str) -> str | None:
    """Обязательное предупреждение, которое нужно показывать НАД графиком (не
    только внутри свернутого expander) — например про peeking или про
    exploratory-природу сегментных разрезов. None, если для этого типа графика
    отдельного предупреждения не предусмотрено."""
    key = _HELP_ALIASES.get(chart_type, chart_type)
    return _WARNINGS.get(key)


def _markdown_lite_to_html(text: str) -> str:
    """Простейший markdown -> HTML: параграфы (разделены пустой строкой) и
    **жирный** текст. Наши тексты помощи не используют ничего сложнее этого."""
