...
/** Same as seedTwoMetricExperiment, but with the metrics declared in the
 * OPPOSITE order — clicks (secondary) first, revenue (primary) second —
 * specifically for the 6-part package pt.3 primary-first-ordering tests:
 * declaring secondary first is what actually exercises the reordering (a
 * primary-metric-declared-first config would pass even without the fix). */
export async function seedTwoMetricExperimentSecondaryDeclaredFirst(
  request: APIRequestContext,
  name: string,
  opts: { email?: string; password?: string } = {},
): Promise<void> {
