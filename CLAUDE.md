...
**Свернута по умолчанию + запоминание выбора** (пакет share+folders): панель
стартует свернутой, а выбор пользователя (развернул/свернул) хранится НА
СЕРВЕРЕ — `users.folders_panel_collapsed` (миграция 0018, `server_default=true`
— это и есть продуктовое «свернута по умолчанию», существующие пользователи
получают его вместе с новыми). Раньше здесь был `useState(false)`: панель и
открывалась всегда развернутой, и забывала выбор при любом уходе со страницы
(`FolderPanel` размонтируется вместе со списком).

Почему сервер, а не localStorage: своего механизма пользовательских настроек в
приложении не было вовсе — это ПЕРВАЯ per-user настройка, и localStorage был бы
первым клиентским стором состояния в проекте, где всё остальное состояние
серверное, плюс он привязан к браузеру, а не к пользователю. Почему
типизированная BOOLEAN-колонка, а не JSONB `preferences`: у `users` весь
остальной флаговый ряд именно такой (`is_active`, `must_change_password`),
типизирован от БД до TS; JSONB ради одного булева был бы первым
бесschema-состоянием. Настройка №2 — такая же аддитивная миграция (правило
(б)); если приедут сразу несколько — тогда и пересмотреть в пользу JSONB.

Путь значения: `users` → `CurrentUser` (`abkit/auth/guards.py` — UI-настройка,
а не право, но CurrentUser и так проекция строки users на запрос, и
`current_user_from_token` эту строку уже читает: иначе `/me` и `/login` ходили
бы в БД второй раз) → `UserOut` (приезжает и с `/login`, и с `/me` — отдельного
запроса за настройками на старте нет) → `AuthContext`. Состояние живет в
`AuthContext` (смонтирован ВЫШЕ роутера в `main.tsx`), а не в компоненте —
поэтому переживает навигацию. `PATCH /auth/me/preferences` — любая роль (это
про свой интерфейс, не про данные), частичный патч (`None` = «не трогать»), БЕЗ
записи в `audit_log`: журнал про изменения данных и прав, «свернул панель» там
был бы шумом. `updatePreferences` в AuthContext — оптимистичный (переключатель
должен срабатывать мгновенно), с откатом и пробросом ошибки наверх: тост
показывает вызывающий компонент.

**Следствие для e2e**: настройка персистентная и per-user, а спеки делят
`admin@e2e.test` — раскрытая одним тестом панель остается раскрытой для
следующего. Отсюда `expandFolderPanel()` в `frontend/e2e/helpers.ts`
(ИДЕМПОТЕНТНЫЙ: раскрывает, только если свернута — слепой клик по «Show
folders» во втором тесте упал бы, кнопки уже нет), а `folders-collapse.spec.ts`
заводит свежую учетку на каждый тест: иначе «по умолчанию свернута» зависело бы
