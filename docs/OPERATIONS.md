...
Admin-панель (Settings → Security → List Users → таб **Monitoring**, роль
Admin) показывает, что ABSet потребляет прямо сейчас и в динамике —
...
графиком из Monitoring-таба за этот период.
...
рост (реальная утечка, аномально большой датасет) убивал и перезапускал
контейнер (`restart: unless-stopped`), а не забирал память соседних
сервисов/хоста целиком. Поднять через `.env`, если реальные датасеты
заметно крупнее тестовых (1.5М строк).
